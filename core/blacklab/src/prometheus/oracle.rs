...
impl ServiceKalman {
    ...
    pub fn predict(&mut self) {
        ...
        self.p = [
            [fp00*self.f[0][0] + fp01*self.f[0][1] + self.q[0][0], fp00*self.f[1][0] + fp01*self.f[1][1] + self.q[0][1]],
            [fp10*self.f[0][0] + fp11*self.f[0][1] + self.q[1][0], fp10*self.f[1][0] + fp11*self.f[1][1] + self.q[1][1]],
        ];
    }
    ...
    pub fn update(&mut self, measured_cpu: f64, measured_mem: f64) {
        ...
        let s = [[self.p[0][0]+self.r[0][0], self.p[0][1]+self.r[0][1]],
        ...
        let k = [[self.p[0][0]*s_inv[0][0] + self.p[0][1]*s_inv[1][0],
        ...
        let ttf_cpu = if drift_cpu > 1e-9 { cpu_headroom / drift_cpu } else { core::f64::INFINITY };
        let ttf_mem = if drift_mem > 1e-9 { mem_headroom / drift_mem } else { core::f64::INFINITY };
...
impl OracleSupervisor {
    pub fn new() -> Self {
        Self { filters: BTreeMap::new(), pre_spawned: BTreeMap::new() }
    }
