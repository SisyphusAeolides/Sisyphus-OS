...
| xHCI reset-ready transport | Integrated prerequisite | Retained live claim; bounded header journal, firmware/no-legacy resolution, SMI-mask readback, port-reset drain, halt, transactional exact BAR0 lease, lease-backed protocol/PSI decode, consuming runtime seed, halted DCBAAP/CRCR/CONFIG/ERST/ERDP preparation, exact translated-DMA mapping receipts with retryable teardown debt (including an all-or-nothing scratchpad pointer-array/buffer pair), read-only root-port census, a Q35 reversible VT-d epoch (fresh disabled-unit proof, real arena, enable/map, program/scrub, revoke/release), and bus-master enable/readback/revoke/restoration; QEMU proves 16 KiB, USB2 4 + USB3 4, two connected ports, and bus mastering off | Persistent scoped VT-d backend, persistent bus-master/Run-Stop transaction, interrupter 0, and USB child enumeration |
