...
| xHCI reset-ready transport | Integrated prerequisite | Retained live claim; bounded header journal, firmware/no-legacy resolution, SMI-mask readback, port-reset drain, halt, transactional exact BAR0 lease, lease-backed protocol/PSI decode, consuming runtime seed, halted DCBAAP/CRCR/CONFIG/ERST/ERDP preparation, exact translated-DMA mapping receipts with retryable teardown debt, read-only root-port census, and a Q35 reversible DMA epoch (fresh disabled-VT-d proof, real arena, program/scrub/reclaim); QEMU proves 16 KiB, USB2 4 + USB3 4, two connected ports, and bus mastering off | Live scoped VT-d backend, bus-master/Run-Stop transaction, interrupter 0, and USB child enumeration |
