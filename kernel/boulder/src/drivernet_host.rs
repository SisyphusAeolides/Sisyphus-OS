...
use crate::drivers::gpu_portability::{
    GpuPortabilityResolver, PortabilityFault, bar_address,
};
...
use crate::drivers::drivernet::registry::{
    ...
    PROBE_EVIDENCE_IDENTITY, PROBE_EVIDENCE_IOMMU_DOMAIN,
    PROBE_EVIDENCE_MMIO_DECODE,
};
use crate::drivers::drivernet::topology::{BootTopologyTable, TopologyTableError};
...
impl BoulderNativeHost {
    ...
    fn sample_bar(
        ...
        let window = MmioWindow::map(physical, 4, device).map_err(|_| {
            BackendFault::new(FaultCode::ProbeFault, true, physical)
        })?;
        ...
        if close_status != sisyphus_driver_abi::STATUS_OK
            || first != second
            || first == u32::MAX
        {
            return Err(BackendFault::new(
    ...
    fn probe(
        ...
        let (evidence_bit, value) = match step.semantic {
            ...
            ProbeSemantic::VerifyIommuIsolation => {
                if !fingerprint.iommu_isolated()
                    || fingerprint.iommu_group == u32::MAX
                {
                    return Err(BackendFault::new(
                        FaultCode::ProbeFault,
                        false,
            ...
            ProbeSemantic::MapControlBarReadOnly => {
                let (bar, sample) = self.sample_bar(
                    fingerprint,
                    usize::from(step.argument.min(5)),
                    device,
                )?;
        ...
        transaction.root = mix(
            transaction.root,
            value ^ u64::from(evidence_bit) ^ tick,
        );
        Ok(ProbeObservation {
...
>(
    ...
    let mut drivernet =
        DriverNet::new_measured(secrets, OraclePolicy::BLACK_LAB, expected_model)?;
    ...
    let nvidia =
        NativeBrokerAdapter::new(DriverStrategy::HermesNvidia, &native_strategy_host)?;
    let amd = NativeBrokerAdapter::new(DriverStrategy::AmdDisplay, &native_strategy_host)?;
    let intel =
        NativeBrokerAdapter::new(DriverStrategy::IntelDisplay, &native_strategy_host)?;
    let virtio =
        NativeBrokerAdapter::new(DriverStrategy::VirtioGpu, &native_strategy_host)?;
    let virtual_svga =
