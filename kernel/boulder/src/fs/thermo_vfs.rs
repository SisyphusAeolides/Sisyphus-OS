...
impl ThermoBlock {
    pub fn new(id: u64, data: Vec<u8>) -> Self {
        ...
        let state = if entropy < 0.3 { ThermalState::Crystalline }
                    else if entropy < 0.7 { ThermalState::Warm }
                    else { ThermalState::Gaseous };
        Self { id, entropy, temperature: 300.0, state, data }
    }
    ...
    fn shannon_entropy(data: &[u8]) -> f64 {
        ...
        freq.iter()
            .filter(|&&f| f > 0)
            .map(|&f| { let p = f as f64 / n; -p * libm::log2(p) })
            .sum::<f64>() / 8.0 // normalize to [0,1]
    }
    ...
    fn recompute_state(&mut self) {
        self.state = if self.temperature > 1000.0 { ThermalState::Crystalline }
                     else if self.temperature > 400.0 { ThermalState::Warm }
                     else { ThermalState::Gaseous };
    }
}
...
impl MaxwellDemon {
    pub fn new() -> Self {
        Self { blocks: BTreeMap::new(), hot_partition: Vec::new(), cold_partition: Vec::new() }
    }
        ...
        if let Some(b) = self.blocks.get(&id) {
            match b.state {
                ThermalState::Crystalline => {
                    if !self.hot_partition.contains(&id) { self.hot_partition.push(id); }
                    self.cold_partition.retain(|&x| x != id);
                }
                ThermalState::Gaseous => {
                    if !self.cold_partition.contains(&id) { self.cold_partition.push(id); }
    ...
    pub fn gc_tick(&mut self, dt: f64) {
        ...
        for id in ids {
            if let Some(b) = self.blocks.get_mut(&id) { b.cool(dt); }
            self.sort_block(id);
        }
    }
}
