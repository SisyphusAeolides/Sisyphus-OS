...
impl AccretionNode {
    ...
    pub fn access(&mut self) {
        if self.is_spaghettified {
            return;
        }
        // Heating up increases orbital radius (pushes back from horizon)
        self.temperature_fp = self
            .temperature_fp
            .saturating_add(XRAY_LUMINOSITY_FP as u64);
        self.radius_fp = self
            .radius_fp
            .saturating_add((XRAY_LUMINOSITY_FP as u64) * 10);
    ...
    pub fn stats(&self) -> VoidStats {
        VoidStats {
            files_in_orbit: self
                .nodes
                .iter()
                .filter(|(_, n)| !n.is_spaghettified)
                .count() as u32,
            files_destroyed: self
                .nodes
                .iter()
                .filter(|(_, n)| n.is_spaghettified)
                .count() as u32,
