...
impl Synapse {
    ...
    pub fn update_stdp(&self, pre_tick: u64, post_tick: u64) {
        ...
        self.weight.fetch_add(delta.max(-50).min(50), Ordering::Relaxed);
    ...
    pub fn decay(&self) {
        ...
        if w > 0 { self.weight.fetch_sub(SYNAPSE_DECAY, Ordering::Relaxed); }
        else if w < 0 { self.weight.fetch_add(SYNAPSE_DECAY, Ordering::Relaxed); }
    }
}
