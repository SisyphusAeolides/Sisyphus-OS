...
impl Atom {
    ...
    pub fn crystallize(&mut self, sender: u32, receiver: u32, type_id: u64,
                       recv_type: u64, data: &[u8], seq: u64, now_ns: u64) -> bool {
        // CAS: empty → writing
        if self.flags.compare_exchange(Self::FLAG_EMPTY, Self::FLAG_WRITING,
    ...
    pub fn shatter(&mut self) -> Option<(&[u8], u64, u32)> {
        if self.flags.compare_exchange(Self::FLAG_READY, Self::FLAG_CONSUMED,
                                        Ordering::AcqRel, Ordering::Relaxed).is_err() {
            return None;
        }
    ...
    fn xxhash32(data: &[u8]) -> u32 {
        ...
        for chunk in data.chunks(4) {
            let mut word = 0u32;
            for (i, &b) in chunk.iter().enumerate() { word |= (b as u32) << (i * 8); }
            h = h.wrapping_add(word.wrapping_mul(PRIME3));
            h = h.rotate_left(17).wrapping_mul(PRIME4);
        }
        h ^= h >> 15; h = h.wrapping_mul(PRIME2);
        h ^= h >> 13; h = h.wrapping_mul(PRIME3);
        h ^= h >> 16;
        h
    }
}
...
impl TransmutationSchema {
    pub fn identity(type_id: u64, name: &str, size: usize) -> Self {
        Self {
            ...
            field_map: vec![FieldMapping { from_offset: 0, to_offset: 0, size, is_inhabited: true }],
            is_zero_copy: true,
    ...
    pub fn validate(&self, payload: &[u8]) -> bool {
        if payload.len() < self.from_size { return false; }
        // Check all field mappings are within bounds
        for field in &self.field_map {
            if !field.is_inhabited { continue; }
            if field.from_offset + field.size > payload.len() { return false; }
        }
        true
    }
    ...
    pub fn transmute(&self, src: &[u8], dst: &mut [u8]) -> bool {
        if !self.validate(src) { return false; }
        if dst.len() < self.to_size { return false; }
        // Zero-initialize destination (prevents uninit reads)
        for b in dst[..self.to_size].iter_mut() { *b = 0; }
        for field in &self.field_map {
            if !field.is_inhabited { continue; }
            let src_end = field.from_offset + field.size;
            let dst_end = field.to_offset + field.size;
            if src_end > src.len() || dst_end > dst.len() { return false; }
            dst[field.to_offset..dst_end]
                .copy_from_slice(&src[field.from_offset..src_end]);
        }
...
impl Crucible {
    pub fn new(receiver_pid: u32, capacity: usize) -> Self {
        ...
        for _ in 0..capacity { atoms.push(Atom::new_empty()); }
        Self {
            receiver_pid,
            atoms,
    ...
    pub fn send(&mut self, sender: u32, type_id: u64, recv_type: u64,
                data: &[u8], now_ns: u64) -> Option<u64> {
        ...
        if !atom.crystallize(sender, self.receiver_pid, type_id, recv_type, data, seq, now_ns) {
...
impl Philosopher {
    ...
    pub fn register(&mut self, pid: u32) {
        self.crucibles.insert(pid, Crucible::new(pid, CRUCIBLE_CAPACITY));
    }
    ...
    pub fn register_schema(&mut self, schema: TransmutationSchema) {
        self.schemas.insert((schema.from_type_id, schema.to_type_id), schema);
    }

    /// Subscribe a process to receive messages of a given type
    pub fn subscribe(&mut self, pid: u32, type_id: u64) {
        self.routing_table.entry(type_id).or_insert_with(Vec::new).push(pid);
    }

    /// Send a message — the Philosopher routes and optionally transmutes it
    pub fn send(&mut self, sender: u32, receiver: u32,
                type_id: u64, data: &[u8]) -> Result<u64, IpcError> {
        let receiver_crucible = self.crucibles.get_mut(&receiver)
        ...
        let seq = receiver_crucible.send(sender, type_id, type_id, data, self.wall_ns)
    ...
    pub fn transmute_send(&mut self, sender: u32, receiver: u32,
                          from_type: u64, to_type: u64, data: &[u8]) -> Result<u64, IpcError> {
        if from_type == to_type {
            return self.send(sender, receiver, from_type, data);
        }

        let schema = self.schemas.get(&(from_type, to_type))
        ...
        let crucible = self.crucibles.get_mut(&receiver)
            .ok_or(IpcError::NoSuchReceiver)?;
        crucible.send(sender, from_type, to_type, &transmuted, self.wall_ns)
            .ok_or(IpcError::BufferFull)
            .map(|seq| { self.total_sent.fetch_add(1, Ordering::Relaxed); seq })
    }

    /// Broadcast: publish to all type subscribers
    pub fn broadcast(&mut self, sender: u32, type_id: u64, data: &[u8]) -> usize {
        let receivers: Vec<u32> = self.routing_table.get(&type_id)
        ...
        for receiver_pid in receivers {
            ...
            if crucible.send(sender, type_id, type_id, data, self.wall_ns).is_some() {
                delivered += 1;
                self.total_sent.fetch_add(1, Ordering::Relaxed);
            } else {
    ...
    pub fn recv(&mut self, pid: u32) -> Option<AlchemyMessage> {
        ...
        Some(AlchemyMessage { data, type_id, sender_pid, latency_ns: latency })
    }
