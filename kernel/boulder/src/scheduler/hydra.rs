...
impl ProcessArm {
    ...
    pub fn compute_reward(
        ...
        let q_granted = if quantum_granted_ns == 0 {
            1
        } else {
            quantum_granted_ns
        };
        ...
        let yield_bonus = if quantum_used_ns < quantum_granted_ns / 2 {
            0.1
        } else {
            0.0
        };
    ...
    pub fn cache_warmth(&self, core_id: u8, now_ns: u64) -> f64 {
        ...
        if age >= CACHE_WARM_THRESHOLD_NS {
            return 0.0;
        }
    ...
    pub fn numa_score(&self, core_numa_node: u8) -> f64 {
        if core_numa_node == self.numa_node {
            1.0
        } else {
            0.3
        }
...
impl CoreBandit {
    ...
    pub fn select_next(&mut self, runnable: &[u32], now_ns: u64) -> Option<(u32, u64)> {
        ...
        self.quantum_end_ns
            .store(now_ns + quantum, Ordering::Relaxed);
    ...
    pub fn adaptive_quantum(&self, pid: u32, now_ns: u64) -> u64 {
        ...
        let ipc_factor = if ipc_factor < 0.5 {
            0.5
        } else if ipc_factor > 2.0 {
            2.0
        } else {
            ipc_factor
        };
...
impl Hydra {
    pub fn new(num_cores: usize) -> Self {
        ...
        let num_cores = if num_cores < MAX_CORES {
            num_cores
        } else {
            MAX_CORES
        };
    ...
    fn best_core_for(&self, pid: u32) -> usize {
        let arm = match self.global_arms.get(&pid) {
            Some(a) => a,
            None => return 0,
        };
        let mut best = 0;
        ...
        for (i, rq) in self.runqueues.iter().enumerate() {
            // Check affinity
            if (arm.affinity_mask & (1u64 << (i % 64))) == 0 {
                continue;
            }
            // Prefer matching NUMA node
            let numa_penalty = if self.cores[i].numa_node != arm.numa_node {
                1000
            } else {
                0
            };
    ...
    pub fn tick(&mut self, now_ns: u64) -> Vec<(u8, u32, u64)> {
        ...
        for i in 0..self.cores.len() {
            ...
            if runnable.is_empty() {
                self.cores[i]
                    .idle_ns
                    .fetch_add(QUANTUM_NS, Ordering::Relaxed);
                continue;
            }
    ...
    pub fn work_steal(&mut self, _now_ns: u64) {
        ...
        let num_cores = if self.cores.is_empty() {
            1
        } else {
            self.cores.len()
        };
        ...
        let idle_cores: Vec<usize> = loads
            .iter()
            .enumerate()
            .filter(|(i, l)| **l == 0 && self.cores[*i].should_steal())
            .map(|(i, _)| i)
            .collect();
        let busy_cores: Vec<usize> = loads
            .iter()
            .enumerate()
        ...
        for idle in &idle_cores {
            if let Some(&busy) = busy_cores.first() {
                ...
                let steal_pid = self.runqueues[busy]
                    .iter()
                    .filter(|&&pid| {
                        let arm = self.global_arms.get(&pid);
                        arm.map(|a| !a.is_pinned && (a.affinity_mask & (1u64 << (*idle % 64))) != 0)
                            .unwrap_or(false)
                    })
                    .cloned()
                    .next();
    ...
    pub fn gossip_round(&mut self) {
        ...
        let snapshots: Vec<BTreeMap<u32, ProcessArm>> =
            self.cores.iter().map(|c| c.local_arms.clone()).collect();
        for i in 0..n {
            let neighbor = (i + 1) % n;
    ...
    pub fn metrics(&self) -> SchedulerMetrics {
        ...
        let avg_posterior: f64 = if self.global_arms.is_empty() {
            0.5
        } else {
            self.global_arms
                .values()
