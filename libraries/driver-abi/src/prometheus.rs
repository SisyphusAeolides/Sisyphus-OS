...
impl PrologueDecoder {
    pub fn decode(bytes: &[u8]) -> Self {
            ...
            match state {
                ProbeState::Start => {
                    match b {
                        ...
                        0xC3 => { uses_red_zone = true; state = ProbeState::Done; i += 1; }
                        _ => { i += 1; }
                    }
                }
                ProbeState::SeenPush | ProbeState::SeenSubRsp => {
                    ...
                    if i + 4 < limit && bytes[i] == 0x48 && bytes[i+1] == 0x89 {
                        match bytes[i+2] {
                            0x7C | 0x74 | 0x55 | 0x4D => { sysv_spills += 1; i += 5; }
                            0x4C | 0x54 | 0x44 | 0x5C => { ms_spills   += 1; i += 5; }
                            _ => { i += 1; }
                        }
                    } else {
                        state = ProbeState::Done;
        ...
        PrologueDecoder { detected, stack_frame_size: stack_frame, saves_shadow: shadow_alloc, uses_red_zone, is_32bit }
    }
}
...
impl ThunkPool {
    pub const fn new() -> Self {
        Self { pool: [0x90u8; THUNK_POOL_LEN], count: 0, total_gen: AtomicU32::new(0) }
    }
    ...
    pub unsafe fn gen_msx64_to_sysv(
        ...
        if self.count as usize >= MAX_THUNKS { return None; }
        ...
        // mov rdi, rcx
        if arg_count >= 1 {
            buf[off..off+3].copy_from_slice(&[0x48, 0x89, 0xCF]); off += 3;
        }
        // mov rsi, rdx
        if arg_count >= 2 {
            buf[off..off+3].copy_from_slice(&[0x48, 0x89, 0xD6]); off += 3;
        }
        // mov rdx, r8
        if arg_count >= 3 {
            buf[off..off+3].copy_from_slice(&[0x4C, 0x89, 0xC2]); off += 3;
        }
        // mov rcx, r9
        if arg_count >= 4 {
            buf[off..off+3].copy_from_slice(&[0x4C, 0x89, 0xCA]); off += 3;
        }

        // Align stack to 16 bytes (SysV requires 16-byte alignment before call)
        // sub rsp, 8
        buf[off..off+4].copy_from_slice(&[0x48, 0x83, 0xEC, 0x08]); off += 4;

        // mov rax, imm64 (target address)
        buf[off] = 0x48; buf[off+1] = 0xB8; off += 2;
        let addr = target_sysv as u64;
        buf[off..off+8].copy_from_slice(&addr.to_le_bytes()); off += 8;

        // call rax
        buf[off..off+2].copy_from_slice(&[0xFF, 0xD0]); off += 2;

        // add rsp, 8
        buf[off..off+4].copy_from_slice(&[0x48, 0x83, 0xC4, 0x08]); off += 4;

        // ret
        buf[off] = 0xC3; off += 1;

        // Pad remainder with int3 (0xCC) for safety
        for b in &mut buf[off..] { *b = 0xCC; }
...
pub fn scan_elf64_for_entry(blob: &[u8]) -> Option<SymbolInfo> {
    if blob.len() < 64 { return None; }
    if &blob[0..4] != ELF_MAGIC { return None; }
    if blob[4] != 2 { return None; } // EI_CLASS = ELFCLASS64
    ...
    if shoff == 0 || shentsize < 64 { return None; }
    ...
    for i in 0..shnum {
        ...
        if sh + shentsize > blob.len() { break; }
    ...
    for s in 0..sym_count {
        ...
        if sym_type != 2 { continue; } // STT_FUNC only
        ...
        for (idx, &target) in DRIVER_ENTRY_SYMBOLS.iter().enumerate() {
            let tlen = target.len();
            if name_ptr + tlen <= blob.len()
                && &blob[name_ptr..name_ptr + tlen] == target
            {
                return Some(SymbolInfo {
                    offset: st_value, size: st_size, name_match: idx as u8
                });
            }
        }
    }
