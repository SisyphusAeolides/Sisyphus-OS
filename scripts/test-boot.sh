...
grep -Fq "Boulder: xHCI reset-ready " "$output"
grep -Fq "bar0-bytes=16384" "$output"
grep -Fq "protocols=2 usb2-ports=4 usb3-ports=4" "$output"
...
grep -Fq "reset-ready=1 aperture-bytes=16384 protocols=2 usb2-ports=4 usb3-ports=4 connected=2 enabled=0 overcurrent=0 debt=0" "$output"
grep -Fq "Boulder: device bindings retained detected=" "$output"
grep -Fq "Boulder: local APIC timer" "$output"
grep -Fq "Boulder: local APIC deadline clock" "$output"
