...
grep -Fq "Boulder: xHCI persistent VT-d/bus-master Run/Stop/reset epoch started/halted/reset-ready start-polls=" "$output"
grep -Fq "Boulder: xHCI reversible DMA epoch prepared/scrubbed/reclaimed regions=4 pages=4" "$output"
grep -Fq "Boulder: xHCI persistent bus-master epoch enabled/readback/runtime/revoked/restored bus-master=false" "$output"
