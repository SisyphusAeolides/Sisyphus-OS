...
impl GestureQueue {
    ...
    fn push(&mut self, ev: GestureEvent) -> bool {
        if self.count >= MAX_GESTURE_QUEUE {
            return false;
        }
...
impl GestureRecognizer {
        ...
        for slot in self.slots.iter_mut() {
            ...
            } = slot.state
            {
                if self.tick.saturating_sub(down_tick) >= HOLD_TICKS {
                    slot.state = PointerState::Holding {
                        x: down_x,
                        y: down_y,
                    };
                    let id = slot.id;
                    self.queue.push(GestureEvent::Hold {
                        x: down_x,
                        y: down_y,
                        pointer_id: id,
                    });
    ...
    pub fn feed(&mut self, event: InputEvent) {
        match event {
            ...
            InputEvent::Scroll { axis: 0, value, .. } => {
                ...
                self.queue.push(GestureEvent::ScrollV {
                    value,
                    x: px,
                    y: py,
                });
            ...
            InputEvent::Scroll { axis: 1, value, .. } => {
                let (px, py) = self.cursor_pos();
                self.queue.push(GestureEvent::ScrollH {
                    value,
                    x: px,
                    y: py,
                });
    ...
    fn on_down(&mut self, id: u8, x: i32, y: i32) {
        ...
        if let Some(slot) = self.slots.iter_mut().find(|s| s.id == id) {
            slot.state = PointerState::Pressed {
                down_x: x,
                down_y: y,
                down_tick: tick,
            };
            ...
            match slot.state {
                PointerState::Pressed { down_x, down_y, .. } => {
                    if slot.max_movement(down_x, down_y, x, y) >= DRAG_THRESHOLD {
                        ...
                        queue.push(GestureEvent::DragBegin {
                            x,
                            y,
                            pointer_id: id,
                        });
                ...
                PointerState::Dragging {
                    ...
                    *last_x = x;
                    *last_y = y;
                    queue.push(GestureEvent::DragUpdate {
                        dx,
                        dy,
                        x,
                        y,
...
mod tests {
    ...
    fn tap_on_short_press_release() {
        ...
        r.feed(InputEvent::TouchDown {
            id: 0,
            x: 100,
            y: 100,
        });
        r.advance_tick(5);
        r.feed(InputEvent::TouchUp {
            id: 0,
            x: 101,
            y: 100,
        });
