...
impl KeybindSettings {
    pub fn bind(&mut self, action: ShellAction, keybind: Keybind) -> bool {
        for e in self.entries[..self.count].iter_mut() {
            if e.action == action { e.keybind = keybind; e.enabled = true; return true; }
        }
        if self.count >= MAX_KEYBINDS { return false; }
        self.entries[self.count] = KeybindEntry { action, keybind, enabled: true };
        self.count += 1;
        true
    }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum VsyncMode { Off, Adaptive, On }
...
impl Default for DisplaySettings {
    fn default() -> Self {
        Self { refresh_hz: 60, vsync: VsyncMode::Adaptive, gamma: 220, hdr: false }
    }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum CrashBehavior { Ignore, ShowNotification, RestartShell }
...
impl SettingsStore {
    ...
    pub fn bind_action(&mut self, action: ShellAction, keybind: Keybind) -> bool {
        ...
        if ok { self.dirty |= dirty::KEYBINDS; }
        ok
    }
    ...
    pub fn clear_dirty(&mut self) { self.dirty = 0; }
    pub fn is_dirty(&self, section: u8) -> bool { self.dirty & section != 0 }
    ...
    pub fn serialize(&self, buf: &mut [u8]) -> Option<usize> {
        ...
        buf[24] = match self.display.vsync { VsyncMode::Off => 0, VsyncMode::Adaptive => 1, VsyncMode::On => 2 };
        ...
        buf[31] = match self.system.crash_behavior {
            CrashBehavior::Ignore => 0, CrashBehavior::ShowNotification => 1,
    ...
    pub fn deserialize(&mut self, buf: &[u8]) -> bool {
        if buf.len() < 34 { return false; }
        const MAGIC: u32 = 0xC4E57077;
        let magic = u32::from_le_bytes([buf[0], buf[1], buf[2], buf[3]]);
        if magic != MAGIC || buf[4] != SETTINGS_VERSION { return false; }
        ...
        self.display.vsync              = match buf[24] { 0 => VsyncMode::Off, 2 => VsyncMode::On, _ => VsyncMode::Adaptive };
        ...
        self.system.crash_behavior      = match buf[31] { 0 => CrashBehavior::Ignore, 2 => CrashBehavior::RestartShell, _ => CrashBehavior::ShowNotification };
        ...
        for _ in 0..kcount.min(MAX_KEYBINDS) {
            ...
            self.keybinds.entries[i] = crate::compositor::settings::KeybindEntry {
                action, keybind: Keybind::new(code, mods), enabled,
            };
