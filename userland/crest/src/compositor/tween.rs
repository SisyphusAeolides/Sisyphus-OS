...
fn ease_in_out_cubic(t: Fixed) -> Fixed {
    ...
    Fixed::from_integer(3).multiply(t2)
...
impl Tween {
    const INACTIVE: Self = Self {
        target_id: 0, start: Fixed::ZERO, end: Fixed::ZERO,
        duration_ticks: 0, elapsed_ticks: 0, easing: EasingFn::Linear,
        active: false, spring_pos: Fixed::ZERO, spring_vel: Fixed::ZERO,
    };

    pub fn new(target_id: u32, start: Fixed, end: Fixed, duration_ticks: u32, easing: EasingFn) -> Self {
        Self {
            target_id, start, end,
            duration_ticks: duration_ticks.max(1),
            elapsed_ticks: 0, easing, active: true,
            spring_pos: start, spring_vel: Fixed::ZERO,
        }
    }
        ...
        match self.easing {
            ...
            _ => {
                ...
                self.start.saturating_add(
                    self.end.saturating_sub(self.start).multiply(eased)
                )
            }
        }
    }
    ...
    fn tick_spring(&mut self, stiffness_raw: i32, damping_raw: i32, delta: u32) -> Fixed {
        ...
        if self.spring_pos.saturating_sub(self.end).abs() <= eps
            && self.spring_vel.abs() <= eps
        {
    ...
    pub fn current(&self) -> Fixed {
        if self.active { self.spring_pos } else { self.end }
    }
}
...
impl TweenBank {
    ...
    pub fn start(&mut self, target_id: u32, start: Fixed, end: Fixed,
                 duration_ticks: u32, easing: EasingFn) -> bool {
    ...
    pub fn gc(&mut self) {
        ...
        for r in 0..self.count {
            if self.tweens[r].active { self.tweens[w] = self.tweens[r]; w += 1; }
        }
        for i in w..self.count { self.tweens[i] = Tween::INACTIVE; }
        self.count = w;
    }
...
mod tests {
    ...
    fn spring_settles_near_target() {
        ...
        let mut t = Tween::new(1, Fixed::ZERO, Fixed::ONE, 500,
                               EasingFn::Spring { stiffness_raw: k, damping_raw: d });
