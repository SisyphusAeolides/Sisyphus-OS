...
pub enum InputEvent {
    ...
    PointerAbs     { x: i32, y: i32, buttons: u8 },
    Scroll         { axis: u8, value: i32, discrete: bool },
    TouchDown      { id: u8, x: i32, y: i32 },
    TouchMove      { id: u8, x: i32, y: i32 },
    TouchUp        { id: u8, x: i32, y: i32 },
    StylusDown     { x: i32, y: i32, pressure: u16 },
    StylusMove     { x: i32, y: i32, pressure: u16, tilt_x: i16, tilt_y: i16 },
    StylusUp       { x: i32, y: i32 },
    PointerDown    { id: u8, x: i32, y: i32 }, // added back so gesture.rs still compiles!
    PointerMove    { id: u8, x: i32, y: i32 }, // added back so gesture.rs still compiles!
    PointerUp      { id: u8, x: i32, y: i32 }, // added back so gesture.rs still compiles!
}
...
impl ModifierState {
    ...
    pub const fn has(self, mask: u16) -> bool { self.0 & mask != 0 }
    pub const fn with(self, mask: u16)    -> Self { Self(self.0 | mask) }
    pub const fn without(self, mask: u16) -> Self { Self(self.0 & !mask) }
    ...
    pub fn update(self, code: u32, pressed: bool) -> Self {
        ...
        if pressed { self.with(bit) } else { self.without(bit) }
    }
}
...
impl KeyRemap {
    pub const fn new() -> Self {
        Self { from: [0; MAX_REMAPS], to: [0; MAX_REMAPS], count: 0 }
    }

    pub fn add(&mut self, from: u32, to: u32) -> bool {
        if self.count >= MAX_REMAPS { return false; }
    ...
    fn remap(&self, code: u32) -> u32 {
        (0..self.count).find(|&i| self.from[i] == code)
            .map(|i| self.to[i])
            .unwrap_or(code)
    }
}
    ...
    fn filter(&mut self, event: InputEvent, _: ModifierState) -> Option<InputEvent> {
        Some(match event {
            InputEvent::Key { code, pressed } =>
                InputEvent::Key { code: self.remap(code), pressed },
            other => other,
        })
    }
}
...
impl PointerAbsIntegrator {
    pub const fn new(width: i32, height: i32) -> Self {
        Self { x: width / 2, y: height / 2, width, height, buttons: 0, replace_motion: true }
    }
        ...
        match event {
            InputEvent::PointerMotion { delta_x, delta_y } => {
                ...
                if self.replace_motion {
                    Some(InputEvent::PointerAbs { x: self.x, y: self.y, buttons: self.buttons })
                } else {
                    Some(event)
                }
            }
...
impl<'f> InputIntegrator<'f> {
    ...
    pub fn add_filter(&mut self, f: &'f mut dyn InputFilter) -> bool {
        if self.filter_count >= MAX_FILTERS { return false; }
    ...
    pub const fn modifiers(&self) -> ModifierState { self.modifiers }
    pub const fn pointer_x(&self) -> i32           { self.pointer_x }
    pub const fn pointer_y(&self) -> i32           { self.pointer_y }
}
...
impl Keybind {
    pub const fn new(code: u32, modifiers: u16) -> Self { Self { code, modifiers } }
    pub const fn plain(code: u32) -> Self { Self { code, modifiers: 0 } }
...
mod tests {
    ...
    fn remap_filter_remaps_caps_to_ctrl() {
        ...
        r.add(58, 29);
        assert_eq!(
            r.filter(InputEvent::Key { code: 58, pressed: true }, ModifierState::empty()),
            Some(InputEvent::Key { code: 29, pressed: true })
        );
    }
    ...
    fn abs_integrator_clamps_to_bounds() {
        ...
        p.warp(1900, 540);
        let r = p.filter(InputEvent::PointerMotion { delta_x: 9999, delta_y: 0 },
                         ModifierState::empty());
        assert_eq!(r, Some(InputEvent::PointerAbs { x: 1919, y: 540, buttons: 0 }));
    }
    ...
    fn keybind_matches_chord() {
        ...
        let ev = IntegratedEvent {
            raw:       InputEvent::Key { code: 28, pressed: true },
